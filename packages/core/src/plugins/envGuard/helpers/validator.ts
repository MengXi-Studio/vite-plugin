 ...
 * - `path`：必须以 `./`、`/`、`\\` 或盘符开头
 */
function validateType(value: string, rule: EnvFieldRule): { valid: boolean; status: 'type_error' | 'enum_mismatch' | 'pass'; message: string } {
	switch (rule.type) {
		case 'number': {
 ...
 * 非 `number` 类型始终返回验证通过。
 */
function validateRange(value: string, rule: EnvFieldRule): { valid: boolean; status: 'range_error' | 'pass'; message: string } {
 ...
 */
function validateLength(value: string, rule: EnvFieldRule): { valid: boolean; status: 'length_error' | 'pass'; message: string } {
 ...
 * 2. **类型验证**：调用 {@link validateType} 检查值的类型
 * 3. **范围验证**：调用 {@link validateRange} 检查数值范围
 * 4. **长度验证**：调用 {@link validateLength} 检查字符串长度
 ...
 * @returns 所有变量的验证结果数组
 *
 * @description 遍历所有规则，对每个环境变量调用 {@link validateValue} 进行验证，
