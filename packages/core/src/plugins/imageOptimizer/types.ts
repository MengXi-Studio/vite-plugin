 ...
 * - `svg`: SVG 矢量格式
 */
export type ImageFormat = 'jpeg' | 'png' | 'webp' | 'avif' | 'gif' | 'tiff' | 'svg'
 ...
 * ```
 */
export type ConvertMapping = Partial<Record<ImageFormat, ImageFormat>>
 ...
 * 不同格式使用不同的质量衡量标准。
 */
export interface FormatQualityOptions {
...
export interface ImageOptimizeSummary {
	...
	totalRatio: number
	/** 按格式分组的统计 */
	byFormat: Record<string, { count: number; originalSize: number; optimizedSize: number; ratio: number }>
	/** 格式转换统计：转换的文件数量 */
 ...
 * ```
 */
export interface ImageOptimizerOptions extends BasePluginOptions {
	/** 各格式的压缩质量参数 */
