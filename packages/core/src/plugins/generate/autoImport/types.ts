 ...
 * ```
 */
export type DirConfig = string | DirConfigObject

/**
 * 目录配置对象
 */
export interface DirConfigObject {
	/** 目录路径或 glob 模式 */
	glob: string
	/** 是否将该目录下的导出作为类型导入 */
	types?: boolean
 ...
 * 目录扫描选项
 */
export interface DirsScanOptions {
 ...
 * ```
 */
export type DtsConfig = string | boolean | DtsConfigObject

/**
 * DTS 配置对象
 */
export interface DtsConfigObject {
	/** 输出文件路径 */
	filepath: string
 ...
 * ```
 */
export interface EslintrcConfig {
	/** 是否启用生成 */
	enabled?: boolean
 ...
 * ```
 */
export interface BiomelintrcConfig {
	/** 是否启用生成 */
	enabled?: boolean
 ...
 * Vue 指令自动导入配置
 */
export interface VueDirectivesConfig {
	/** 是否启用 Vue 指令自动导入 */
	enabled?: boolean
	/** 判断导入项是否为指令的函数 */
 ...
 * ```
 */
export interface InlineImportConfig {
	/** 源模块路径 */
	from: string
