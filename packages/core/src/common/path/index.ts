 ...
 * ```
 */
export function isExtensionIncluded(ext: string, options: { includeExtensions?: string[]; excludeExtensions?: string[] }): boolean {
 ...
 * ```
 */
export function isPathExcluded(relativePath: string, excludePaths: string[], options: { matchMode?: 'simple' | 'segment' } = {}): boolean {
	if (excludePaths.length === 0) return false

	const normalizedPath = normalizePath(relativePath)
	const { matchMode = 'simple' } = options
 ...
 * ```
 */
export function isPreCompressed(ext: string): boolean {
