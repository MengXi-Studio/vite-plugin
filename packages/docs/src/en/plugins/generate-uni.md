...
Page config can be declared per-page via the `defineUniPage` macro or the `<route-config>` block, exactly the same as `generatePages`. The macro has higher priority than `<route-config>`, and its global type declaration is auto-generated by the plugin (customize via `pages.dts`), see the [macro docs in generatePages](./generate-pages#defineunipage-macro):
...
You can also use the `<route-config>` block (lower priority than the macro; adding `lang="jsonc"` gives IDE highlighting):
...
| pages | [`GeneratePagesOptions`](/en/plugins/generate-pages) | `{ pagesDir: 'src/pages', subPackages: [{ root: 'pages-sub', dir: 'src/pages-sub' }] }` | Phase 1 options: scan pages + `<route-config>` / `defineUniPage` to generate pages.json (`pagesDir` / `subPackages` / `routeConfigBlock` / `entryPage` / `titleFallback` / `tabBar` / `includeExtensions` / `excludePatterns` / `dts`) |
...
- Virtual module requests for the `<route-config>` block are intercepted and replaced with an empty module, so the build won't try to parse them as JavaScript; `defineUniPage` macro calls are also removed at transform time, so no runtime import is needed
