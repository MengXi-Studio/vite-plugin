...
在 Vite 构建完成后自动优化图片，支持 JPEG、PNG、WebP、AVIF、GIF、TIFF、SVG 格式的压缩与格式转换。
...
::: tip 两个依赖均为可选，插件会自动检测已安装的依赖并仅执行对应的优化任务。 :::
...
| keepOriginal     | `boolean`                                                            | `false`                                                                                          | 格式转换时是否保留原始文件             |
| reportOutput     | `string` \| `false`                                                  | `'image-optimize-report.json'`                                                                   | 优化报告输出路径，false 不生成         |
| parallelLimit    | `number`                                                             | `10`                                                                                             | 并发优化最大文件数                     |
...
- `keepOriginal: false`（默认）时格式转换会替换原始文件
- `sharp` 和 `svgo` 均为可选依赖，未安装时插件会自动跳过对应类型的优化
- 并发优化通过 `parallelLimit` 控制资源占用，默认 10 个并发
