...
<tbody>
${n.slice(0,20).map(i=>{const u=i.trend==="increased"?"diff-positive":i.trend==="decreased"?"diff-negative":i.trend==="added"?"diff-added":i.trend==="removed"?"diff-removed":"diff-unchanged",m=i.trend==="increased"?"&#9650;":i.trend==="decreased"?"&#9660;":i.trend==="added"?"&#10010;":i.trend==="removed"?"&#10006;":"&#9644;";return`<tr><td>${i.name}</td><td>${i.previousSize>=0?common_format_index.formatFileSize(i.previousSize):"-"}</td><td>${i.currentSize>=0?common_format_index.formatFileSize(i.currentSize):"-"}</td><td class="${u}">${i.diff>=0?"+":""}${common_format_index.formatFileSize(Math.abs(i.diff))} (${i.diffPercentage}%)</td><td class="${u}">${m} ${i.trend}</td></tr>`}).join("")}
</tbody>
</table>
</div>
`:""}
