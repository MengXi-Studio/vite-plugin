...
- [index.ts（工厂插件入口）](file://packages/core/src/factory/plugin/index.ts#L1-L387)
...
## 核心组件
- Validator：提供链式 API 的参数验证器，支持必填、类型检查、默认值设置与自定义验证。
- BasePlugin：插件基类，负责配置合并、日志、错误策略与验证器初始化；子类通过重写 validateOptions 完成业务验证。
...
P->>V : "new Validator(mergedOptions)"
P->>S : "validateOptions()"
...
### 与工厂与插件体系的集成
- BasePlugin
  - 构造阶段：创建 Validator，调用 validateOptions，随后进入生命周期。
...
## 依赖分析
- 组件耦合
  - BasePlugin 依赖 Validator 与 deepMerge；Validator 与业务插件解耦，仅依赖传入的配置对象。
...
## 结论
Validator 以简洁的链式 API 提供了强健的配置验证能力，结合 BasePlugin 的默认值合并与错误策略，形成了从“配置到执行”的闭环保障。通过内置类型检查、自定义规则与默认值填充，开发者可以快速构建可靠、易维护的插件配置体系。
