...
## 更新摘要
**所做变更**
- 新增 format 工具模块，包含 padNumber、generateRandomHash、getDateFormatParams、formatDate、parseTemplate 等格式化函数
...
- 格式化工具：提供数字补零、随机哈希生成、日期格式化与模板解析等格式化功能
- 对象工具：提供深度合并对象的能力
...
```mermaid
graph TB
FORMAT["Format 工具<br/>format.ts"] --> CP["CopyFilePlugin<br/>copyFile/index.ts"]
VAL["Validator<br/>validation.ts"] --> CP
...
### 格式化工具（format）
- 功能概览
  - padNumber：数字补零格式化，支持指定目标长度
  - generateRandomHash：生成指定长度的随机哈希字符串，支持1-64字符范围
  - getDateFormatParams：获取日期格式化参数对象，包含年月日时分秒毫秒和时间戳
  - formatDate：基于模板格式化日期字符串
  - parseTemplate：解析模板字符串，替换占位符
- 关键流程（日期格式化）
```mermaid
flowchart TD
Start(["开始"]) --> GetParams["获取日期参数<br/>getDateFormatParams"]
GetParams --> CreateTemplate["创建模板字符串<br/>{YYYY}-{MM}-{DD}"]
CreateTemplate --> ReplaceYear["替换 {YYYY} 为四位年份"]
ReplaceYear --> ReplaceMonth["替换 {MM} 为两位月份"]
ReplaceMonth --> ReplaceDay["替换 {DD} 为两位日期"]
ReplaceDay --> ReplaceHour["替换 {HH} 为两位小时"]
ReplaceHour --> ReplaceMinute["替换 {mm} 为两位分钟"]
ReplaceMinute --> ReplaceSecond["替换 {ss} 为两位秒数"]
ReplaceSecond --> ReplaceMillisecond["替换 {SSS} 为三位毫秒"]
ReplaceMillisecond --> ReplaceTimestamp["替换 {timestamp} 为时间戳"]
ReplaceTimestamp --> End(["结束"])
```

**图表来源**
- [packages/core/src/common/format.ts](file://packages/core/src/common/format.ts#L76-L88)
- [packages/core/src/common/format.ts](file://packages/core/src/common/format.ts#L104-L113)

- API 参考
  - padNumber(num: number, length: number = 2): string
    - 作用：数字补零格式化，将数字转换为指定长度的字符串
    - 参数：num - 要格式化的数字；length - 目标长度，默认2
    - 返回：补零后的字符串
    - 示例：padNumber(5, 2) → '05'，padNumber(123, 2) → '123'
  - generateRandomHash(length: number = 8): string
    - 作用：生成指定长度的随机哈希字符串
    - 参数：length - 哈希长度，范围1-64，默认8
    - 返回：随机哈希字符串
    - 异常：超出长度范围时自动调整到有效范围
    - 示例：generateRandomHash(8) → 'a1b2c3d4'
  - getDateFormatParams(date: Date = new Date()): DateFormatOptions
    - 作用：获取日期格式化参数对象
    - 参数：date - 日期对象，默认当前时间
    - 返回：包含YYYY、YY、MM、DD、HH、mm、ss、SSS、timestamp的参数对象
    - 示例：getDateFormatParams(new Date()) → { YYYY: '2026', MM: '02', DD: '03', ... }
  - formatDate(date: Date, format: string): string
    - 作用：根据模板格式化日期字符串
    - 参数：date - 日期对象；format - 格式模板，支持{YYYY}、{MM}、{DD}等占位符
    - 返回：格式化后的日期字符串
    - 示例：formatDate(new Date(), '{YYYY}-{MM}-{DD}') → '2026-02-03'
  - parseTemplate(template: string, values: Record<string, string>): string
    - 作用：解析模板字符串，替换占位符
    - 参数：template - 模板字符串；values - 占位符值映射
    - 返回：替换后的字符串
    - 示例：parseTemplate('{name}-{version}', { name: 'app', version: '1.0.0' }) → 'app-1.0.0'
- 类型定义
  - DateFormatOptions
    - YYYY: string - 四位年份
    - YY: string - 两位年份
...
- [packages/core/src/common/fs/index.ts](file://packages/core/src/common/fs/index.ts#L193-L243)
...
- API 参考
  ...
  - ensureTargetDir(targetPath: string): Promise<void>
    ...
    - 返回：Promise<void>
    - 异常：权限不足或其他 IO 错误
  - readDirRecursive(dirPath: string, recursive: boolean): Promise<FileEntry[]>
    - 作用：读取目录内容，支持递归
    - 参数：dirPath - 目录路径；recursive - 是否递归
    - 返回：Promise<FileEntry[]> 文件/目录绝对路径数组
...
**章节来源**
- [packages/core/src/common/object.ts](file://packages/core/src/common/object.ts#L35-L67)
...
- API 参考
  ...
  - validate(): T
    - 作用：执行验证，失败抛出错误
- 使用示例（来自插件）
  - 插件内部使用示例路径：[packages/core/src/plugins/copyFile/index.ts](file://packages/core/src/plugins/copyFile/index.ts#L22-L40)
...
**图表来源**
- [packages/core/src/factory/plugin/index.ts](file://packages/core/src/factory/plugin/index.ts#L253-L311)
...
- 使用示例（来自测试）
  - 基本使用与高级配置示例路径：[packages/core/src/plugins/copyFile/index.ts](file://packages/core/src/plugins/copyFile/index.ts#L95-L114)
...
- API 参考
  - generateIconTagDescriptors(options: IconOptions): HtmlTagDescriptor[]
  - IconOptions
    - base?: string
    - url?: string
    - link?: string
...
**章节来源**
- [packages/core/src/plugins/injectIco/common/index.ts](file://packages/core/src/plugins/injectIco/common/index.ts#L10-L60)
...
## 依赖分析
- 模块内依赖
  - 复制文件插件依赖：fs 工具、format 工具、验证器、日志系统、工厂基类
  - 注入图标工具独立于 fs 工具，但可与复制文件插件组合使用
- 外部依赖
  - Node.js fs/path/crypto；Vite 插件系统；TypeScript 类型系统
- 依赖关系图
```mermaid
graph LR
FORMAT["format.ts"] --> CP["copyFile/index.ts"]
VAL["validation.ts"] --> CP
FS["fs/index.ts"] --> CP
...
- [packages/core/src/common/format.ts](file://packages/core/src/common/format.ts#L1)
...
- 格式化性能
  - padNumber 和 generateRandomHash 为 O(1) 操作，性能开销极小
  - formatDate 使用正则替换，复杂度与模板占位符数量线性相关
  - parseTemplate 为 O(n) 操作，其中 n 为模板字符串长度
- 递归读取
...
- 格式化错误
  - 检查 padNumber 的 length 参数是否为正数
  - verify generateRandomHash 的 length 是否在1-64范围内
  - 确认模板字符串中的占位符是否匹配
- 验证失败
...
**章节来源**
- [packages/core/src/common/format.ts](file://packages/core/src/common/format.ts#L33-L38)
...
- [packages/core/src/logger/index.ts](file://packages/core/src/logger/index.ts#L116-L130)
- [packages/core/src/plugins/copyFile/index.ts](file://packages/core/src/plugins/copyFile/index.ts#L58-L80)

## 结论
工具模块通过清晰的职责划分与统一的工厂模式，提供了稳定可靠的文件复制、格式化、对象合并、参数验证与日志输出能力。新增的格式化工具模块增强了字符串处理能力，包括数字补零、随机哈希生成、日期格式化与模板解析等功能。插件基于这些工具快速构建，具备良好的扩展性与可维护性。建议在复杂场景下结合增量复制与合理的日志策略，以及格式化工具的高效字符串处理能力，以获得更好的开发体验与构建效率。
