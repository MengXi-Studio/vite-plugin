...
### 日志系统（logger）
- 功能概览
  - Logger：统一输出前缀（库名+插件名+时间戳）、四种级别（info/success/warn/error）、彩色与图标、受 enabled 控制
- API 参考
...
- [packages/test/src/copyFile/copyFile.test.ts](file://packages/test/src/copyFile/copyFile.test.ts#L39-L218)
- [packages/test/src/injectIco/injectIco.test.ts](file://packages/test/src/injectIco/injectIco.test.ts#L50-L271)