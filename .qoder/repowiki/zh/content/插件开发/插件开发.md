...
## 项目结构
本项目采用多包工作区组织，核心能力集中在 packages/core 中，包含插件工厂、通用工具、日志与验证模块，以及三个内置插件（复制文件、注入图标、生成版本号）。
...
L["src/plugins/generateVersion/index.ts<br/>生成版本号插件"]
end
A --> I
B --> C
C --> E
...
- [packages/core/src/plugins/generateVersion/types.ts](file://packages/core/src/plugins/generateVersion/types.ts#L27-L120)
- [packages/core/src/logger/index.ts](file://packages/core/src/logger/index.ts#L7-L188)
- [packages/core/src/common/validation.ts](file://packages/core/src/common/validation.ts#L16-L203)
...
### 内置插件：生成版本号（generateVersion）
- **配置项**：format/customFormat/semverBase/autoIncrement/outputType/outputFile/defineName/hashLength/prefix/suffix/extra/继承基础配置。
- **生命周期**：在 buildEnd 阶段生成版本号，支持文件输出和代码注入。
- **验证**：使用 Validator 校验格式类型与输出配置。
- **错误处理**：通过 safeExecute 包裹，按 errorStrategy 处理。
...
GenVer["GenerateVersionPlugin"] --> FS
...
## 结论
本插件工厂体系以 BasePlugin 为核心，通过统一的生命周期、配置合并、验证、日志与错误处理，大幅降低了插件开发复杂度。内置 copyFile、injectIco 与 generateVersion 展示了典型场景的实现方式。遵循本文档的流程与最佳实践，开发者可快速扩展出满足自身需求的高质量 Vite 插件。
...
### 自定义插件开发流程（从需求到部署）
- **需求分析**
  - 明确插件目标（如：构建后处理、HTML 注入、静态资源复制、版本号生成等）。
  - 识别 Vite 钩子（如：configResolved、transformIndexHtml、writeBundle、buildEnd 等）。
- **设计与建模**
...
- **生成版本号插件（generateVersion）**
  - 场景：在构建结束时生成版本号并输出到文件或注入代码。
  - 关键点：buildEnd 钩子生成版本号；支持多种格式与输出类型。
  - 参考路径：[生成版本号插件类型定义](file://packages/core/src/plugins/generateVersion/types.ts#L27-L120)
...
- **GenerateVersionOptions**：生成版本号插件配置接口
...
- **generateVersion 插件测试**：验证版本号生成、格式转换、输出类型
